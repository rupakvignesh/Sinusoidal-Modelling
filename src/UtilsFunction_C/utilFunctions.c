...
{
    int ii,jj, min_ind;
    float Ponddif, FreqDistance, MagFactor;
...
{
    float Amax, min_val, maxMeasuredFreq, minF0Candidate, *ErrorPM, *ErrorMP, *pmag_local, **measMTX, **predMTX;
    ...
    predMTX = (float **)malloc(sizeof(float*)*nf0c);
    measMTX = (float **)malloc(sizeof(float*)*nf0c);
    
    for(ii=0;ii<nf0c;ii++)
    {
        predMTX[ii] = (float*)malloc(sizeof(float)*canMTXLen);
        for(jj=0;jj<canMTXLen;jj++)
        {
            predMTX[ii][jj] = (jj+1)*f0c[ii];
        }
        
        measMTX[ii] = (float *)malloc(sizeof(float)*nPeaks);
    ...
    ErrorPM = (float*)malloc(sizeof(float)*nf0c);
    memset(ErrorPM, 0, sizeof(float)*nf0c);
    ErrorMP = (float*)malloc(sizeof(float)*nf0c);
    memset(ErrorMP, 0, sizeof(float)*nf0c);
    pmag_local = (float*)malloc(sizeof(float)*nPeaks);
